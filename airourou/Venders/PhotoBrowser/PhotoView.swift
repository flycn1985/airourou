...
class GJPhotoView: UIScrollView, UIScrollViewDelegate {
    ...
    func showImage(animated animated: Bool) {
        ...
        imageView.ar_setImageWithURL(self.url.replace("!original.jpg", withString: "!full.jpg"))
        setupImageViewFrame()
        if !animated {
            beginDownload()
        } else {
    ...
    private func setupGesture() {
        let doubleTapGes = UITapGestureRecognizer(target: self, action: #selector(GJPhotoView.handleDoubleTap(_:)))
        doubleTapGes.numberOfTapsRequired = 2
        self.addGestureRecognizer(doubleTapGes)
        
        let singleTapGes = UITapGestureRecognizer(target: self, action: #selector(GJPhotoView.handleSingleTap))
        self.addGestureRecognizer(singleTapGes)
    }
    
    private func beginDownload() {
        let exists = SDWebImageManager.sharedManager().cachedImageExistsForURL(NSURL(string: url))
        if !exists {
            loadingView.hidden = false
            loadingView.progress = 0.1
        }
        
        imageView.sd_setImageWithURL(NSURL(string: url), placeholderImage:UIImage(named: "image_placehoder.png"), options: .LowPriority, progress: { [weak self] (receivedSize: Int, totalSize: Int) -> Void in
            ...
            UIView.animateWithDuration(photoViewConst.animationDuration, animations: { () -> Void in
                ...
                self.backgroundColor = UIColor.clearColor()
                    ...
                    if let delegate = self.photoViewDelegate {
                        if delegate.respondsToSelector(#selector(GJPhotoViewDelegate.photoViewDidSingleTap(_:))) {
